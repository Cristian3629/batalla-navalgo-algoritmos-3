...
public abstract class Barco extends ObjetoObservable implements Movible, Destructible, Observador {
    ...
    public Barco(int tam, int vida, MovimientoStrategy estrategia, Element nodoBarco) {
        tamanio = tam;
        ...
        List<Element> listaNodosPartes = nodoBarco.element("Partes").elements();
        if (listaNodosPartes.size() != tam) {
            throw new Error("Cantidad de partes: " + listaNodosPartes.size() + " no corresponde al barco." + ((List<Element>) (nodoBarco.elements())).get(0).getName());
        }
    ...
    public void actualizar() {
        if (this.estaDestruido()) {
            ...
            Parte parteAux;
            for (int i = 0; i < partesDelBarco.size(); i++) {
                parteAux = partesDelBarco.get(i);
                tablero.sacarElemento(parteAux.obtenerPosicion(), parteAux);
            }
        }
    }
    ...
    private void sacarPartes() {
        Tablero tablero = Tablero.getTablero();
        for (int i = 0; i < tamanio; i++) {
            Vector posParte = new Vector(posicionCabeza.sumar(orientacion.porEscalar(i)));
    ...
    public Element generarNodo(String nombreBarco) {
        ...
        nodoADevolver.add(estrategia.generarNodo());
        Element nodoPartes = DocumentHelper.createElement("Partes");
        Iterator<Parte> it = partesDelBarco.iterator();
        while (it.hasNext()) {
            Parte parteActual = it.next();
            nodoPartes.add(parteActual.generarNodo());

        }
        nodoADevolver.add(nodoPartes);
