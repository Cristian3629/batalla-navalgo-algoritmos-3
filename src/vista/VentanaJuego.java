...
public class VentanaJuego extends Ventana implements Observador {
	...
	private void inicializarModelo(String dirArchivo) throws IOException {
		...
		if (!dirArchivo.equals("")) {
			...
			ArrayList<VistaDaniador> vistasDaniadores = (new ConstructorDeDaniador())
					.construirVistasDaniadores(nodoDisparos, daniadores,
							dibujante);
			for (int h = 0; h < vistasDaniadores.size(); h++) {
				(vistasDaniadores.get(h)).agregarObservador(this);
			}
		} else {
			partida = new Partida();
